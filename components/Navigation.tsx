...
const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
  ...
  return (
    <Link
      href={href}
      className={
        `rounded-full px-4 py-2 text-sm font-medium transition-colors duration-200 sm:rounded-none sm:px-0 sm:py-4 sm:pr-2 ${isActive ? "bg-blue-500 text-white shadow-md sm:bg-transparent sm:text-blue-600 sm:shadow-none" : "bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-slate-200 sm:bg-transparent sm:text-gray-600 sm:hover:bg-transparent sm:hover:text-blue-500 sm:dark:bg-transparent sm:dark:text-slate-700"}` ||
